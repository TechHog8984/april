...
int outputClass(Class& _class, std::string& output) {
    ...
    for (uint16_t i = 0; i < _class.method_count; i++) {
        ...
        std::string localfunc_name = "method";
        localfunc_name.append(std::to_string(i));
        localfunc_name.push_back('_');
        for (uint16_t i = 0; i < method.name->Utf8.bytes_size; i++) {
            char ch = method.name->Utf8.bytes[i];
            localfunc_name.push_back((ch == '<' || ch == '>' || ch == '$') ? '_' : ch);
        }
        ...
        if (!is_abstract) {
            ...
            if (is_native) {
                ...
                for (uint32_t pc = 0; pc < code_attribute->Code.code_count; pc++) {
                    ...
                    // FIXME: test: monitorenter, monitorexit, dload_0, dload_1, dload_2, dload_3, ldc_w, invokespecial (on InterfaceMethodref), invokestatic (on InterfaceMethodref), l2i, dup_x1, dup_x2, dup2, dup2_x1, dup2_x2, lneg, dconst_0, dconst_1, dcmpl, dcmpg, dstore_0, dstore_1, dstore_2, dstore_3, land, lor, lshl, lshr, dreturn, d2i, i2s, d2f, freturn, lushr, dadd, i2d, ddiv, laload, saload, daload, dstore, dload, faload, fstore, fload, fcmpg, fcmpl, lastore, sastore, dastore, fastore,  irem, lmul, f2d, dsub, ldiv, lrem, fstore_0, fstore_1, fstore_2, fstore_3, multianewarray, wide, l2d, fdiv, fadd, dneg, fsub, fneg, f2l, l2f, jsr, ret, drem, frem
                    ...
                    auto doop_ldc = [&output, &old_pc](Constant& constant) {
                        ...
                        switch (constant.tag) {
                            case ConstantType::Class:
                                ...
                                indent(output);
                                output.append("local classobject = class.classobject\n");
                                if (disable_codegen_asserts == 0) {
                                    indent(output);
                                    output.append("assert(classobject, \"class \" .. classname .. \" has no Class object!\")\n");
                                }
                                indent(output);
                    ...
                    auto doop_iload = [&output, &old_pc, code_attribute, getLocalVariableComment] (uint16_t index) {
                        ...
                        indent(output);
                        output.append("local newvalue = april.intJavaValue(value)\n");
                        if (disable_codegen_asserts == 0) {
                            indent(output);
                            output.append("assert(newvalue, \"invalid value (not integer type) in local array index ")
                                .append(index_str)
                    ...
                    auto doop_istore = [&output, &old_pc, code_attribute] (uint16_t index) {
                        ...
                        if (disable_codegen_asserts == 0) {
                            indent(output);
                            output.append("newvalue = assert(newvalue, \"invalid value (not integer type) at the top of the stack in instruction istore; \" .. value.tag)\n");
                        }
                        indent(output);
                    ...
                    switch (code) {
                        ...
                        OPCONDITIONAL(0x1a, iload_0)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) in local array index 1 in instruction iload_0; \" .. value.tag)\n");
                            }
                            indent(output);
                            output.append("store(stack, newvalue)\n");
                        ...
                        OPCONDITIONAL(0x1b, iload_1)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) in local array index 2 in instruction iload_1; \" .. value.tag)\n");
                            }
                            indent(output);
                            output.append("store(stack, newvalue)\n");
                        ...
                        OPCONDITIONAL(0x1c, iload_2)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) in local array index 3 in instruction iload_2; \" .. value.tag)\n");
                            }
                            indent(output);
                            output.append("store(stack, newvalue)\n");
                        ...
                        OPCONDITIONAL(0x1d, iload_3)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) in local array index 4 in instruction iload_3; \" .. value.tag)\n");
                            }
                            indent(output);
                            output.append("store(stack, newvalue)\n");
                        ...
                        OPCONDITIONAL(0x3b, istore_0)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) at the top of the stack in instruction istore_0; \" .. value.tag)\n");
                            }
                            indent(output);
                        ...
                        OPCONDITIONAL(0x3c, istore_1)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) at the top of the stack in instruction istore_1; \" .. value.tag)\n");
                            }
                            indent(output);
                        ...
                        OPCONDITIONAL(0x3d, istore_2)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) at the top of the stack in instruction istore_2; \" .. value.tag)\n");
                            }
                            indent(output);
                        ...
                        OPCONDITIONAL(0x3e, istore_3)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(newvalue, \"invalid value (not integer type) at the top of the stack in instruction istore_3; \" .. value.tag)\n");
                            }
                            indent(output);
                        ...
                        OPCONDITIONAL(0x99, ifeq)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in ifeq was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9a, ifne)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in ifne was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9b, iflt)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in iflt was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9c, ifge)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in ifge was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9d, ifgt)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in ifgt was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9e, ifle)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(intvalue, \"value in ifle was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0x9f, if_icmpeq)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmpeq was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmpeq was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0xa0, if_icmpne)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmpne was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmpne was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0xa1, if_icmplt)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmplt was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmplt was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0xa2, if_icmpge)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmpge was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmpge was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0xa3, if_icmpgt)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmpgt was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmpgt was not an integer\")\n");
                            }
                        ...
                        OPCONDITIONAL(0xa4, if_icmple)
                            ...
                            if (disable_codegen_asserts == 0) {
                                indent(output);
                                output.append("assert(value1int, \"value1 in if_icmple was not an integer\")\n");
                                indent(output);
                                output.append("assert(value2int, \"value2 in if_icmple was not an integer\")\n");
                            }
